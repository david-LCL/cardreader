...
/* Define 3 additional GPIO pins for nrf24l01's <CSN, CE, IRQ> */
#define MY_CSN (GPIO_PORT_QS | GPIO_PIN3)
#define MY_AN0 (GPIO_PORT_AN | GPIO_PIN0)	// this is CE pin
#define MY_IRQ7 (GPIO_PORT_NQ | GPIO_PIN7)

/* Define two output port and one input port for <CSN, CE, IRQ> */
FILE_PTR output_port = NULL;
FILE_PTR input_port = NULL;

const uint_32 output_set[] =
{
	MY_CSN | GPIO_PIN_STATUS_1,
	MY_AN0 | GPIO_PIN_STATUS_1,
	GPIO_LIST_END
};

const uint_32 input_set[] =
{
	MY_IRQ7 | GPIO_PIN_STATUS_1,
	GPIO_LIST_END
};

/* Define ports csn, ce, irq */
static const uint_32 my_csn[] = 
{
	MY_CSN,
	GPIO_LIST_END
};

static const uint_32 my_an0[] = 
{
	MY_AN0,
	GPIO_LIST_END
};

static const uint_32 my_irq7[] = 
{
	MY_IRQ7,
	GPIO_LIST_END
};
...
/*  
	This delay function ultizied MQX time functions
	Implement for M52259DEMO
*/
...
void my_delay_us(int us)
{
	...
	_time_add_usec_to_ticks(&ticks, us);
	_time_delay_for(&ticks);
}

/***** START IMPLEMENTING FUNCTIONS FROM HERE *****/
...
// init gpio ports
   ...
   output_port = fopen("gpio:write", (char_ptr) &output_set);
    ...
    if (NULL == output_port) 
   {
      printf ("Error opening output GPIO driver!\n");
      _time_delay (200L);
      _mqx_exit (-1);
   }
    ...
    if (NULL == input_port) 
   {
      printf ("Error opening input GPIO driver!\n");
      _time_delay (200L);
      _mqx_exit (-1);
   }
////////////
...
void nrf24l01_clear_ce()
{
	...
	ioctl(output_port, GPIO_IOCTL_WRITE_LOG0, (pointer) &my_an0);
}
...
{
	// nrf24l01_CE_IOREGISTER |= nrf24l01_CE_PINMASK;
	ioctl(output_port, GPIO_IOCTL_WRITE_LOG1, (pointer) &my_an0);
}
...
void nrf24l01_clear_csn()
{
	...
	ioctl(output_port, GPIO_IOCTL_WRITE_LOG0, (pointer) &my_csn);
}
...
{
	// nrf24l01_CSN_IOREGISTER |= nrf24l01_CSN_PINMASK;
	ioctl(output_port, GPIO_IOCTL_WRITE_LOG1, (pointer) &my_csn);
}
